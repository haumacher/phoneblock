...
-- Per-subject rate-limit counters for expensive API calls. See db-migration-43.sql
-- for the rationale. The subject is an API token (SUBJECT_KIND=1, SUBJECT_ID=TOKENS.ID)
-- or, for account-password authenticated CardDAV, the user (SUBJECT_KIND=0,
-- SUBJECT_ID=USERS.ID). No foreign key: rows are cleaned up explicitly on deletion.
