...
public interface Users {
	...
	@Insert("insert into USERS (LOGIN, DISPLAYNAME, LOCALE, PWHASH, REGISTERED, MIN_VOTES, MAX_LENGTH) " + 
			"values (#{login}, #{displayName}, #{lang}, #{pwhash}, #{registered}, 4, 2000)")
	void addUser(String login, String displayName, String lang, byte[] pwhash, long registered);
	...
	@Select("select ID, LOGIN, DISPLAYNAME, LOCALE, EMAIL, MIN_VOTES, MAX_LENGTH, WILDCARDS, LASTACCESS, CREDIT from USERS where LOGIN=#{login}")
	DBUserSettings getSettingsRaw(String login);
	
	@Select("select LOGIN from USERS where ID=#{userId}")
	String getUserName(long userId);
	
	@Select("select ID, LOGIN, DISPLAYNAME, LOCALE, EMAIL, MIN_VOTES, MAX_LENGTH, WILDCARDS, LASTACCESS, CREDIT from USERS where ID=#{userId}")
	 ...
	 */
	@Select("""
			select u.ID, u.LOGIN, u.DISPLAYNAME, u.LOCALE, u.EMAIL, u.MIN_VOTES, u.MAX_LENGTH, u.WILDCARDS, u.LASTACCESS, u.CREDIT from USERS u
	...
	@Select("select u.ID, u.LOGIN u.DISPLAYNAME, u.LOCALE, u.EMAIL, u.MIN_VOTES, u.MAX_LENGTH, u.WILDCARDS, u.LASTACCESS, u.CREDIT from USERS u "
			+ "where not u.WELCOME "
